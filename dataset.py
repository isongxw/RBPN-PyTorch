...
def load_img(filepath, nFrames, scale, other_dataset):
    ...
    if other_dataset:
        ...
        target = modcrop(Image.open(
            join(filepath, 'im'+str(nFrames)+'.png')).convert('RGB'), scale)
        input = target.resize(
            (int(target.size[0]/scale), int(target.size[1]/scale)), Image.BICUBIC)
...
def get_patch(img_in, img_tar, img_nn, patch_size, scale, nFrames, ix=-1, iy=-1):
    ...
    img_nn = [j.crop((iy, ix, iy + ip, ix + ip))
              for j in img_nn]  # [:, iy:iy + ip, ix:ix + ip]
...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.patch_size != 0:
            input, target, neigbor, _ = get_patch(
                input, target, neigbor, self.patch_size, self.upscale_factor, self.nFrames)
